...
char mqtt_clientid[] = "bsn-arduinosz";
...
unsigned long currmillis, task2, task3, task4;
void loop() {
  currmillis = millis();
  ...
  if((unsigned long)(currmillis - task4) >= 10000){
    act(true);
    client.publish("/bsn/sz/bett/heardbeat", String(random(8)).c_str());
    task4 = millis();
    act();
  }
}
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if((strcmp(topic, "/bsn/sz/bett/rgb/control")==0) || (strcmp(topic, "/bsn/sz/bett/white")==0)){
    ...
    if(strcmp(topic, "/bsn/sz/bett/white")==0){
    ...
    if(strcmp(topic, "/bsn/sz/bett/rgb/control")==0){
      String r = json["r"];
      String g = json["g"];
      String b = json["b"];
      msg;
  ...
  // switch RGB ON
  else if(strcmp(topic, "/bsn/sz/bett/on")==0){
...
void mqtt_reconnect() {
  ...
  while (!client.connected()) {
    ...
    if ( client.connect(mqtt_clientid, mqtt_user, mqtt_password) ) {
      // json values
      client.subscribe("/bsn/sz/bett/rgb/control");
      client.subscribe("/bsn/sz/bett/white");
      // bool values
      client.subscribe("/bsn/sz/bett/on");
      client.subscribe("/bsn/sz/bett/autolight");
    } else {
      delay(5000);
    }
  }
